...
let FileFetcher = {
    _is_fetching: false,
    _queue: [] as { fullname: string; start_line: number; end_line: number }[],
    _fetch: function (fullname: any, start_line: any, end_line: any) {
        ...
        if (FileOps.is_missing_file(fullname)) {
            ...
            console.warn(
                `tried to fetch a file known to be missing ${fullname}`,
            );
        ...
        if (typeof fullname !== "string") {
            console.warn(
                `trying to fetch filename that is not a string`,
                fullname,
            );
        ...
        Util.get_json("/read_file", data)
            ...
            .finally(() => {
                ...
                FileFetcher._queue = FileFetcher._queue.filter(
                    (o) => o.fullname !== fullname,
                );
...
const FileOps = {
    ...
    _store_change_callback: function () {
        ...
        let source_code_selection_state = store.get(
                "source_code_selection_state",
            ),
            fullname = null,
            is_paused = false,
        ...
        if (
            ...
            is_paused = false;
            paused_addr = null;
            require_cached_line_num = parseInt(
                store.get("line_of_source_to_flash"),
            );
        } else if (
            source_code_selection_state ===
        ...
        let source_code_infinite_scrolling = store.get(
            ...
            start_line,
            end_line;
        ({ start_line, end_line, require_cached_line_num } =
            FileOps.get_start_and_end_lines(
                fullname,
    ...
    get_start_and_end_lines(
        ...
        if (source_code_infinite_scrolling) {
            ...
            let source_file_obj =
                FileOps.get_source_file_obj_from_cache(fullname);
            ...
            start_line = Math.max(
                Math.floor(
                    require_cached_line_num -
                        store.get("max_lines_of_code_to_fetch") / 2,
                ),
                1,
            );
            end_line = Math.ceil(
                start_line + store.get("max_lines_of_code_to_fetch"),
            );
            ...
            if (source_file_obj) {
                end_line = Math.ceil(
                    Math.min(
                        end_line,
                        FileOps.get_num_lines_in_file(fullname, null),
                    ),
            ...
            if (start_line > end_line) {
                start_line = Math.floor(
                    Math.max(
                        1,
                        end_line - store.get("max_lines_of_code_to_fetch"),
                    ),
                );
            }
            require_cached_line_num = Math.min(
                require_cached_line_num,
                end_line,
            );
        }
    ...
    update_source_code_state(
        ...
        const states = constants.source_code_states,
            line_is_cached = FileOps.line_is_cached(
                fullname,
                require_cached_line_num,
                null,
            );
        ...
        if (fullname && line_is_cached) {
            ...
            store
                ...
                .some(
                    (obj: any) =>
                        parseInt(obj.address, 16) === parseInt(paused_addr, 16),
                )
        ) {
    ...
    lines_are_cached: function (fullname: any, start_line: any, end_line: any) {
        ...
        const num_lines_in_file = FileOps.get_num_lines_in_file(
            fullname,
            source_file_obj,
        );
            ...
            if (
                ...
                Actions.show_modal(
                    ...
                    <div>
                        ...
                        <span className="bold">after</span> the binary was
                        compiled. Recompile the binary, then try again.
    ...
    save_new_assembly: function (mi_assembly: any, mi_token: any) {
        ...
        for (let cached_file of cached_source_files) {
            if (cached_file.fullname === fullname) {
                cached_file.assembly = Object.assign(
                    cached_file.assembly,
                    assembly_to_save,
                );

                // @ts-expect-error ts-migrate(2345) FIXME: Argument of type 'string[]' is not assignable to p... Remove this comment to see the full error message
                let max_assm_line = Math.max(Object.keys(cached_file.assembly)),
                    max_source_line = Math.max(
                        // @ts-expect-error ts-migrate(2345) FIXME: Argument of type 'string[]' is not assignable to p... Remove this comment to see the full error message
                        Object.keys(cached_file.source_code_obj),
                    );
