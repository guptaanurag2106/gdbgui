...
class Memory extends React.Component<{}, State> {
    ...
    get_memory_component_jsx_content() {
        ...
        let bytes_per_line =
            parseInt(store.get("bytes_per_line")) ||
            Memory.DEFAULT_BYTES_PER_LINE;
    ...
    static set_inputs_from_address(addr: any) {
        ...
        store.set(
            ...
            "0x" +
                (
                    parseInt(addr, 16) + Memory.DEFAULT_ADDRESS_DELTA_BYTES
                ).toString(16),
        );
        Memory.fetch_memory_from_state();
    }
    ...
    static get_gdb_commands_from_state() {
        ...
        if (Number.isInteger(start_addr) && end_addr) {
            ...
            while (cur_addr <= end_addr) {
                ...
                cmds.push(
                    `-data-read-memory-bytes ${"0x" + cur_addr.toString(16)} 1`,
                );
                cur_addr = cur_addr + 1;
            }
        }
    ...
    static make_addrs_into_links_react(text: any) {
        ...
        if (text && matches && matches.length) {
            ...
            let trailing_text = text.slice(
                text.indexOf(addr) + addr.length,
                text.length,
            );
            ...
            if (trailing_text) {
                ...
                suffix_component =
                    Memory.make_addrs_into_links_react(trailing_text);
            }
            return (
                <React.Fragment>
                    {leading_text}
