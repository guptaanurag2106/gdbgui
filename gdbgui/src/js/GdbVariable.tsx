...
let ChildVarFetcher = {
    ...
    _is_fetching: false,
    _queue: [] as { expr_of_parent: any; expr_type: any }[], // objects with keys 'expr_gdb_parent_var_currently_fetching_children' and 'expr_type'
    _fetch_next_in_queue: function () {
        ...
        if (ChildVarFetcher._queue.length) {
            ...
            GdbApi.run_gdb_command(
                `-var-list-children --all-values "${obj.expr_of_parent}"`,
            );
        } else {
            ChildVarFetcher.expr_gdb_parent_var_currently_fetching_children =
                null;
    ...
    fetch_children(expr_of_parent: any, expr_type: any) {
        ChildVarFetcher._queue.push({
            expr_of_parent: expr_of_parent,
            expr_type: expr_type,
        });
...
let VarCreator = {
    ...
    _fetch_next_in_queue: function () {
        ...
        if (VarCreator._queue.length) {
            ...
            let var_create_cmd =
                constants.CREATE_VAR_STR + `-var-create - * ${expression}`;
    ...
    created_variable(r: any) {
        ...
        if (expr) {
            ...
            //      "value": "0"
            //  },
            GdbVariable.save_new_expression(
                expr,
...
class GdbVariable extends React.Component {
            ...
            if (mi_obj.children.length > 0) {
                for (let child of mi_obj.children) {
                    if (child.numchild > 0) {
                        content.push(
                            <li key={child.exp}>
                                {this.get_ul_for_var_with_children(
                                    child.exp,
                                    child,
                                    expr_type,
                                )}
                        ...
                        content.push(
                            <li key={child.exp}>
                                {this.get_ul_for_var_without_children(
                                    child.exp,
                                    child,
                                    expr_type,
                                )}
    ...
    static gdb_created_children_variables(r: any) {
        ...
        let parent_obj = GdbVariable.get_obj_from_gdb_var_name(
            expressions,
            parent_name,
        );
        if (parent_obj) {
    ...
    static prepare_gdb_obj_for_storage(obj: any, parent: any) {
        ...
        new_obj.numchild = obj.dynamic
            ? parseInt(obj.has_more)
            : parseInt(obj.numchild);
    ...
    static _update_numeric_properties(obj: any) {
        let value = obj.value;
        const stringTypeRe =
            /^(?:const\s+)?(?:char|signed\s+char|unsigned\s+char|wchar_t)\s*(?:\*|\[[^\]]*\])(?:\s+const)?$/; // TODO: cpp string type
    ...
    static _update_radix_values(obj: any) {
        if (obj.is_int) {
            ...
            obj._int_value_to_str_in_radix = obj._int_value_decimal.toString(
                obj._radix,
            );
            if (obj._radix === 16) {
                obj._int_value_to_str_in_radix =
    ...
    static hide_children_in_ui(gdb_var_name: any) {
        let expressions = store.get("expressions"),
            obj = GdbVariable.get_obj_from_gdb_var_name(
                expressions,
                gdb_var_name,
            );
        if (obj) {
    ...
    static _toggle_children_visibility(gdb_var_name: any) {
        ...
        if (obj) {
            ...
            console.error(
                "developer error - expected to find gdb variable object",
            );
        }
    }
    ...
    static handle_changelist(changelist_array: any) {
        for (let changelist of changelist_array) {
            ...
            if (obj) {
                if (
                    parseInt(changelist["has_more"]) === 1 &&
                    "name" in changelist
                ) {
                    // already retrieved children of obj, but more fields were added.
                    // Re-fetch the object from gdb
                    ChildVarFetcher.fetch_children(
                        changelist["name"],
                        obj.expr_type,
                    );
                }
                if ("new_children" in changelist) {
                    let new_children = changelist.new_children.map(
                        (child_obj: any) =>
                            GdbVariable.prepare_gdb_obj_for_storage(
                                child_obj,
                                obj,
                            ),
                    );
    ...
    static get_obj_from_gdb_var_name(expressions: any, gdb_var_name: any) {
        ...
        let top_level_var_name =
                GdbVariable.get_root_name_from_gdbvar_name(gdb_var_name),
            children_names =
                GdbVariable.get_child_names_from_gdbvar_name(gdb_var_name);

        let objs = expressions.filter(
            (v: any) => v.name === top_level_var_name,
        );
        ...
        if (objs.length === 1) {
            ...
            for (let i = 0; i < children_names.length; i++) {
                ...
                let child_obj = GdbVariable.get_child_with_name(
                    obj.children,
                    name_to_find,
                );
