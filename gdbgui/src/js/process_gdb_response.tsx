    ...
    for (let r of response_array) {
        ...
        if (isError(r)) {
            if (isCreatingVar(r)) {
                ...
                r.payload &&
                r.payload.msg.indexOf(
                    "Mode argument must be 0, 1, 2, or 3.",
                ) !== -1
            ) {
        ...
        if (r.type === "result" && r.message === "done" && r.payload) {
            ...
            if ("thread-ids" in r.payload) {
                Threads.update_thread_ids(r.payload["thread-ids"]["thread-id"]);
            }
            if ("threads" in r.payload) {
                store.set("threads", r.payload.threads);
                store.set(
                    "current_thread_id",
                    parseInt(r.payload["current-thread-id"]),
                );
            }
            ...
            if ("register-values" in r.payload) {
                store.set(
                    ...
                    store.get("current_register_values"),
                );
                store.set(
            ...
            if ("files" in r.payload) {
                if (r.payload.files.length > 0) {
                    ...
                    let source_file_paths = [
                        ...new Map(
                            r.payload.files.map((f: any) => [f.fullname, f]),
                        ).keys(),
                    ].sort();
                    ...
                    if (source_file_paths.some((p: any) => p.endsWith(".rs"))) {
                        ...
                        source_file_paths.some((p: any) => p.endsWith(".go"))
                    ) {
                        language = "go";
                    }
                    ...
                    if (store.get("inferior_binary_path")) {
                        ...
                        Modal.render(
                            "Warning",
                            <div>
                                This binary was not compiled with debug symbols.
                                Recompile with the -g flag for a better
            ...
            if (
                "has_more" in r.payload &&
                "numchild" in r.payload &&
                "children" in r.payload
            ) {
        ...
        if (r.message && r.message === "stopped") {
            if (r.payload && r.payload.reason) {
                if (r.payload.reason.includes("exited")) {
                    ...
                    console.warn(
                        "TODO handle new reason for stopping. Notify developer of this.",
                    );
                    console.warn(r);
                }
            } else {
