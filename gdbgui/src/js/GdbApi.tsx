...
const GdbApi = {
    ...
    _connect_with_timeout: function (
        url: string,
        timeout_ms = 5000,
    ): Promise<WebSocket> {
        return new Promise((resolve, reject) => {
            ...
            const timer = setTimeout(() => {
                ...
                reject(
                    new Error(
                        `Socket connection timed out after ${timeout_ms}ms`,
                    ),
                );
            }, timeout_ms);
    ...
    init: async function () {
        ...
        try {
            socket = await GdbApi._connect_with_timeout(
                ws_url,
                TIMEOUT_SEC * 1000,
            );
            ...
            Actions.show_modal(
                ...
                <>
                    <p>
                        ...
                        backend is running and refresh the page
                    </p>
                </>,
            );
                    ...
                    case "debug_session_connection_event":
                        {
                            ...
                            const started_new_gdb_process =
                                gdb_pid_obj.started_new_gdb_process;
            ...
            if (event.reason === "existing_connection") {
                Actions.show_modal(
                    ...
                    <>
                        <p>
                            The connection to the gdb session has been closed,
                            as there was an already existing connection.
                        </p>
                    </>,
                );
            } else {
                Actions.show_modal(
                    ...
                    <>
                        <p>
                            The connection to the gdb session has been closed.
                            This tab will no longer function as expected.
                        </p>
                    </>,
                );
            }
    ...
    inferior_is_paused: function () {
        return (
            ...
            ].indexOf(store.get("inferior_program")) !== -1
        );
    },
    ...
    click_gdb_cmd_button: function (e: {
        currentTarget: {
            dataset: { [x: string]: any; cmd: undefined; cmd0: undefined };
        };
    }) {
    ...
    _get_refresh_state_for_pause_cmds: function () {
        ...
        cmds.push(
            constants.IGNORE_ERRORS_TOKEN_STR + "-var-update --all-values *",
        );
