...
class Graph extends React.Component<Props, State> {
    ...
    private _apply_font() {
        const size = Math.max(
            8,
            Math.min(this.internal_width, this.internal_height) / 65,
        );
        this.font_size = size;
    ...
    private _render_static(view?: [number, number]) {
        ...
        if (this.data_x_range[0] === this.data_x_range[1]) {
            ...
            this.data_x_range =
                x === 0 ? [-5, 5] : [Math.min(0, 2 * x), Math.max(0, 2 * x)];
        }
        if (this.data_y_range[0] === this.data_y_range[1]) {
            const y = this.data_y_range[0];
            this.data_y_range =
                y === 0 ? [-5, 5] : [Math.min(0, 2 * y), Math.max(0, 2 * y)];
        }
        ...
        for (let i = view[0]; i <= view[1]; i++) {
            this.screen_coords[i - view[0]] = {
                ...
                sy:
                    h -
                    this.pad_y -
                    (data[i][1] - this.data_y_range[0]) * scale_y,
                di: i,
            };
        }

        this._draw_grid_and_labels(
            octx,
            w,
            h,
            step_x,
    ...
    private _draw_grid_and_labels(
        ...
        for (let k = 0; k <= BARS; k++) {
            ...
            ctx.lineTo(
                Math.round(pos_x) + 0.5,
                Math.round(h - this.pad_y) + 0.5,
            );
        }
        for (let k = 0; k <= BARS; k++) {
            ...
            ctx.lineTo(
                Math.round(w - this.pad_x) + 0.5,
                Math.round(pos_y) + 0.5,
            );
        }
        ctx.stroke();
    ...
    private _hover_point(mx: number, my: number) {
        ...
        const data = this.props.data;
        const r =
            Math.min(this.internal_width, this.internal_height) /
            CIRCLE_RADIUS_RATIO;
        ...
        if (best !== -1 && best_dist <= r * 2) {
            ...
            if (
                ...
                this.internal_width - hover_font
            ) {
