...
class SourceCode extends React.Component<{}, State> {
    ...
    constructor(props: {}) {
        ...
        this.is_gdb_paused_on_this_line =
            this.is_gdb_paused_on_this_line.bind(this);
    }
    ...
    get_body() {
        ...
        switch (this.state.source_code_state) {
            ...
            case states.SOURCE_CACHED: {
                let obj = FileOps.get_source_file_obj_from_cache(
    ...
    click_gutter(line_num: any) {
        Breakpoints.add_or_remove_breakpoint(
            this.state.fullname_to_render,
            line_num,
        );
    }
    ...
    _get_source_line(
        ...
        if (assembly_for_line) {
            ...
            for (let assm of assembly_for_line) {
                assembly_content.push(
                    SourceCode._get_assm_content(i, assm, paused_addr),
                );
        ...
        return (
            <tr
                ref={
                    assign_scroll_to_line_ref ? SourceCode.scroll_to_line : null
                }
    ...
    get_body_source_and_assm(
        ...
        let bkpt_lines = Breakpoints.get_breakpoint_lines_for_file(
            ...
            disabled_breakpoint_lines =
                Breakpoints.get_disabled_breakpoint_lines_for_file(
                    this.state.fullname_to_render,
                ),
        ...
        while (line_num_being_rendered <= end_linenum_to_render) {
            ...
            let has_bkpt = bkpt_lines.indexOf(line_num_being_rendered) !== -1,
                has_disabled_bkpt =
                    disabled_breakpoint_lines.indexOf(
                        line_num_being_rendered,
                    ) !== -1,
                has_conditional_bkpt =
                    conditional_breakpoint_lines.indexOf(
                        line_num_being_rendered,
                    ) !== -1,
    ...
    static is_source_line_visible(ref: React.RefObject<any>) {
        ...
        const el_code_container_rect =
            this.el_code_container.current.getBoundingClientRect();
        let top_of_container = el_code_container_rect.top,
            ...
            top_of_table = ref.current
                .closest("table")
                .getBoundingClientRect().top,
            is_visible =
                top_of_line >= top_of_container &&
                bottom_of_line <= bottom_of_container;
     ...
     */
    static _make_selector_visible(
        selector: React.RefObject<HTMLTableRowElement>,
    ) {
        ...
        if (selector.current) {
            ...
            if (!is_visible) {
                ...
                let scroll_top =
                    top_of_line - (top_of_table + height_of_container / 2);
